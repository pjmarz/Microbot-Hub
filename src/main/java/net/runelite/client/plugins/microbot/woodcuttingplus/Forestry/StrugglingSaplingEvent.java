...
import net.runelite.client.plugins.microbot.woodcuttingplus.AutoWoodcuttingPlusScript;
...
public class StrugglingSaplingEvent implements BlockingEvent {
    ...
    public StrugglingSaplingEvent(AutoWoodcuttingPlusPlugin plugin) {
        this.plugin = plugin;
    }
        ...
        try{
            ...
            return strugglingSaplings.stream().anyMatch(obj ->
                    Rs2GameObject.hasAction(obj.getObjectComposition(), "Add-mulch") &&
                            obj.getWorldLocation().distanceTo(Rs2Player.getWorldLocation()) <= AutoWoodcuttingPlusScript.FORESTRY_DISTANCE
            );
        } catch (Exception e) {
